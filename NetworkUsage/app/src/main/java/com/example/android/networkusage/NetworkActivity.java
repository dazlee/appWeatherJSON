...
import com.example.android.networkusage.WeatherJsonParser;
...
public class NetworkActivity extends Activity {
    ...
    private static final String URL =
            "http://192.168.1.5:3000/weatherJSON";
    ...
    public List<myEntry> parseJSON(InputStream in) {
        List<myEntry> entries = new ArrayList<myEntry>();
        Log.v("NewworkActivity", "parseJSON");
        Reader reader = null;
        try{
            Log.v("NewworkActivity", "reader");
            reader = new InputStreamReader(in, "UTF-8");
        } catch (IOException e) {
            Log.v("NewworkActivity", "reader error");
        }
        char[] buffer = new char[5000];
        try{
            Log.v("NewworkActivity", "buffer");
            reader.read(buffer);
        } catch (IOException e) {
            Log.v("NewworkActivity", "buffer error");
        }

        String jsonString = new String(buffer);
        try{
            Log.v("NewworkActivity", "json");
            JSONObject obj = new JSONObject(jsonString);
            JSONArray location = new JSONArray(obj.getString("location"));
            String city = location.getJSONObject(0).getString("name");
            entries.add(new myEntry(city, "set cloudy here because we need to parse data properly on server"));
        } catch (JSONException e) {
            Log.v("NewworkActivity", "json parse error:" + jsonString);
        }
    ...
    public static class myEntry {
        ...
        protected myEntry(String city, String forecast) {
            this.city = city;
